...
export class TreeReader {
    ...
    private async formatStructGeo(files: any) {
        ...
        for (const file of files) {
            ...
            if (this.fileFunctions[`${folderName}`]) {
                ...
                const fileContent = await this.fileFunctions[`${folderName}`](pathRead) || [];
                ...
                if (projectsFiles[projectName][".csv"] && projectsFiles[projectName][".dxf"]) {
                    let polygons = await this.buildPolygons(projectsFiles, projectName, geo);
    ...
    private async getDme(path: string) {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            if (groups[currentIndexGroup]["name"] !== name) {
                const okBuildCentroid = this.buildCentroid(groups[currentIndexGroup], polygonPoints, path);
                if (!okBuildCentroid) {
                    break;
                } else {
                    polygonPoints = [];
    ...
    private async getStock(path: any) {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            const [name, x, y, z, area, volume, height, level] = files[i].split(/[;,]/)
            if (!name || !x || !y || !z || !volume || !height || !level) {
                this.setErrResponse(path);
                break;
            }
    ...
    private async getMaterial(path: any) {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            const [type, description, crs, orezone, law, thins, cf, place] = files[i].split(';')
            if (!type || !place) {
                this.setErrResponse(path);
                break;
            }
            ...
            parser.parseStream(fileStream, (err: any, dxf: any) => {
                ...
                for (let i = 0; i < entities.length; i++) {
                    ...
                    entities[i].vertices.forEach((v: any) => {
                        const {longitude, latitude, altitude} = this.transformUTM(v.x, v.y, v.z);
                        polygon.point.push({longitude, latitude, altitude});
                        polygonPoints.push([longitude, latitude]);
                    });
                    polygon.altitude = polygon.point[0].altitude;

                    const okBuildCentroid = this.buildCentroid(polygon, polygonPoints, path);
                    if (!okBuildCentroid) {
    ...
    private async getCsvPidExtra(path: string) {
        const OUTPUT_LABELS = ['name', 'material', 'lito', 'alte', 'orezonet', 'au', 'ag', 'as', 'volume', 'tonnes', 'onzas', 'aucn', 'agcn', 'ptaucn', 's']
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            const group = {} as any;
            if (colSplit[2] !== '') {
                if (!colSplit[0] || !colSplit[1] || !colSplit[8] || !colSplit[9] || !colSplit[10]) {
                    this.setErrResponse(path);
                    break;
                }
                for (let i = 0; i < OUTPUT_LABELS.length; i++) {
                    group[OUTPUT_LABELS[i]] = this.isNum(colSplit[i]) ? parseFloat(colSplit[i]) : colSplit[i];
                }
            } else {
                if (!colSplit[3] || !colSplit[4] || !colSplit[11] || !colSplit[12] || !colSplit[13]) {
                    this.setErrResponse(path);
                    break;
                }
                for (let i = 3; i < OUTPUT_LABELS.length + 3; i++) {
                    group[OUTPUT_LABELS[i - 3]] = this.isNum(colSplit[i]) ? parseFloat(colSplit[i]) : colSplit[i];
                }
            }
            polygons.push(group);
        }
    ...
    private async buildPolygons(projectsFiles: any, projectName: string, geo: any) {
        let csvPidContent: any, dxfPidContent: any;
        try {
            csvPidContent = await this.getCsvPidExtra(projectsFiles[projectName][".csv"]);
            dxfPidContent = await this.getDxfPid(projectsFiles[projectName][".dxf"], projectName);
        } catch (e) {
            console.log(e)
            csvPidContent = []
            dxfPidContent = []
        ...
        for (let i = 0; i < dxfPidContent.length; i++) {
            const extra = csvPidContent.find((item: { name: any; }) => dxfPidContent[i].name === item.name);
    ...
    private getFolderName(path: string): string {
        ...
        const entryMatch = valuesFolderName.find(([, value]: any) => pathFormatted.includes(value));
    ...
    private cleanRepeatedCell(geo: any, type: string) {
        ...
        for(let i in geo[`${type}`][0].cell) {
            if (lookupObject[geo[`${type}`][0].cell[i]["name"]]){
                if(lookupObject[geo[`${type}`][0].cell[i]["name"]]["point"].length < geo[`${type}`][0].cell[i]["point"].length){
                    lookupObject[geo[`${type}`][0].cell[i]["name"]] = geo[`${type}`][0].cell[i];
                }
            }else{
                lookupObject[geo[`${type}`][0].cell[i]["name"]] = geo[`${type}`][0].cell[i];
            }
    ...
    private cleanRepeatedPoint(geo: any, type: string) {
        ...
        for(let i in geo[`${type}`]) {
            if (lookupObject[geo[`${type}`][i]["name"]]){
                if(lookupObject[geo[`${type}`][i]["name"]]["point"].length < geo[`${type}`][i]["point"].length){
